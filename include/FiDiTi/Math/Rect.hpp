...
struct Rect
{
    ...
    Vec<D, T> point(std::size_t i) const
    {
        assert(i < (1 << D));
        auto p = min, d = size();
...
template <std::size_t N> using RectNi = Rect<N, int>;
template <std::size_t N> using RectNu = Rect<N, unsigned>;
template <std::size_t N> using RectNf = Rect<N, float>;
template <std::size_t N> using RectNd = Rect<N, double>;
