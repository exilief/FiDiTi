...
class Builder
{
 ...
 public:
    ...
    auto create()
    {
        return lsDomainType::New(bounds, bcs, grid_dx);
    }

    // Horizontal plane
    void makePlane(lsDomainType ls, T height, int orientation = 1)
    {
        T origin[D] = {};  origin[D-1] = height;
        T normal[D] = {};  normal[D-1] = orientation;
    ...
    void cutHoles(lsDomainType plate, int numHoles, T holeWidth, T minX, T maxX, T bottom, T height)
    {
    ...
    void combine(lsDomainType target, lsDomainType add)
    {
        viennals::BooleanOperation<T, D>(
            target, add, viennals::BooleanOperationEnum::UNION)
            .apply();
    }
    ...
    {
        viennals::BooleanOperation<T, D>(
            target, bound, viennals::BooleanOperationEnum::INTERSECT)
            .apply();
    }
